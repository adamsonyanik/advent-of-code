...
function getHeightAfter(iterations: number) {
    ...
    const heightNonCycle: number = start.split("").map(n => Number(n)).reduce((a, b) => a + b);
    const cycleTimes: number = Math.floor((iterations - start.length) / cycle.length);
    const cycleHeight: number = cycle.split("").map(n => Number(n)).reduce((a, b) => a + b);
    ...
    const cycleRemainingHeight: number = cycleRemainingHeightString.split("").map(n => Number(n)).reduce((a, b) => a + b);
