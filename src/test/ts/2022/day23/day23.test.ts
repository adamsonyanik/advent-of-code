...
function draw() {
    ...
    for (let y = minY; y <= maxY; y++) {
        ...
        for (let x = minX; x <= maxX; x++) {
            if (elves.filter(e => e.x == x && e.y == y).length > 0)
...
function simulatePossiblePos(it: number) {
    ...
    for (const elv of elves) {
        ...
        } else if (it < 1 && !isElfAt(elv.x - 1, elv.y - 1) && !isElfAt(elv.x, elv.y - 1) && !isElfAt(elv.x + 1, elv.y - 1)) {
            newElf.y--;
        } else if (it < 2 && !isElfAt(elv.x - 1, elv.y + 1) && !isElfAt(elv.x, elv.y + 1) && !isElfAt(elv.x + 1, elv.y + 1)) {
            newElf.y++;
        } else if (it < 3 && !isElfAt(elv.x - 1, elv.y - 1) && !isElfAt(elv.x - 1, elv.y) && !isElfAt(elv.x - 1, elv.y + 1)) {
            newElf.x--;
        } else if (it < 4 && !isElfAt(elv.x + 1, elv.y - 1) && !isElfAt(elv.x + 1, elv.y) && !isElfAt(elv.x + 1, elv.y + 1)) {
            newElf.x++;
...
function isElfSurrounded(posx: number, posy: number) {
    for (let x = -1; x <= 1; x++) {
        for (let y = -1; y <= 1; y++) {
            ...
            if (elves.filter(e => e.x == x + posx && e.y == y + posy).length > 0)
                return true;
        }
    }
