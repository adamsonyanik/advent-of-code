...
function moveBlizzards() {
    for (const b of blizzards) {
        ...
        if (b.x == 0) b.x = map[0].length - 2;

        if (b.y == 0) b.y = map.length - 2;

        if (b.x == map[0].length - 1) b.x = 1;

        if (b.y == map.length - 1) b.y = 1;
    }
}
...
function getAllOptions(sort: (a: { x: number; y: number }, b: { x: number; y: number }) => number) {
    ...
    for (const pos of possiblePos) {
        for (const p of getOptions(pos)) if (opt.filter((a) => a.x == p.x && a.y == p.y).length == 0) opt.push(p);
    }

    const size = 100;
    if (opt.length > size) return opt.sort(sort).slice(0, size);
...
function getOptions(pos: { x: number; y: number }) {
    ...
    if (
        pos.x - 1 >= 0 &&
        blizzards.filter((b) => b.x == pos.x - 1 && b.y == pos.y).length == 0 &&
        map[pos.y][pos.x - 1] != "#"
    ) {
        options.push({ x: pos.x - 1, y: pos.y });
    }
    if (
        pos.x + 1 < map[0].length &&
        blizzards.filter((b) => b.x == pos.x + 1 && b.y == pos.y).length == 0 &&
        map[pos.y][pos.x + 1] != "#"
    ) {
        options.push({ x: pos.x + 1, y: pos.y });
    ...
    if (
        pos.y - 1 >= 0 &&
        blizzards.filter((b) => b.x == pos.x && b.y == pos.y - 1).length == 0 &&
        map[pos.y - 1][pos.x] != "#"
    ) {
